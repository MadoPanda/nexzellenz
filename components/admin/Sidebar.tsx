  ...
  return (
    <aside className="glass-panel" style={{ 
      ...
      <div style={{ marginBottom: '3rem', padding: '0 1rem' }}>
        <h2 style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: '1.5rem', color: 'white', letterSpacing: '2px' }}>
      ...
      <nav style={{ display: 'grid', gap: '0.5rem', flex: 1 }}>
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              {item.icon}
              <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '0.85rem' }}>{item.name}</span>
            </Link>
          );
        })}
      </nav>
      ...
      <div style={{ marginTop: 'auto', borderTop: '1px solid var(--border)', paddingTop: '1.5rem' }}>
        ...
        >
          <LogOut size={20} />
          <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '0.85rem' }}>Logout</span>
        </button>
      </div>
